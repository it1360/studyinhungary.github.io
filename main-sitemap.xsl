			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>موقعیت نقشه سایت - تحصیل در مجارستان</title>
					</xsl:when>
					<xsl:otherwise>
						<title>نقشه سایت XML - تحصیل در مجارستان</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="https://www.study-hungary.ir/sitemap_index.xml">&#8592; Sitemap Index</a>							</p>
